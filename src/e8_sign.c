	...
	const int32_t *r0, const int32_t *r1)
{
	...
	for (size_t u = 0; u < n; u ++) {
		...
		assert(((uint64_t)w0[u] & 1u) == h0b[u]);
		assert(((uint64_t)w1[u] & 1u) == h1b[u]);
		assert((x0 % 2) == 0);
		assert((x1 % 2) == 0);
	...
	int64_t *trace_pnorm, unsigned *trace_attempts)
{
	...
	/*
	 * Experimental floating-point bounded/truncated path.  This is not
	 * constant-time and is not parameter-calibrated.  It is separate from
	 * the dummy signing helpers above and from all ordinary HAWK signing.
	 */
	...
	size_t n = (size_t)1 << logn;
	double threshold = 8.0 * (double)n * sigma_verify * sigma_verify;
	hash_to_h(logn, h, sc_data, hpub, salt, salt_len);
	...
	for (unsigned attempt = 0; attempt < max_attempts; attempt ++) {
		...
		if (!e8_sample_z_float(z0, z1, &pnorm, t0, t1, logn,
			sigma_sign, sampler_bound, rng, rng_context))
		{
			return 0;
		}
		...
		if (sr == 0 || (double)pnorm > threshold) {
			continue;
		}
